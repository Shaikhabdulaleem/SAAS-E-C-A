...
function buildNavGroups(user: User | null) {
  const isSuperAdmin = !user || user.role === 'superadmin';
  const services = user?.enabledServices ?? [];
  const has = (s: string) => isSuperAdmin || services.includes(s as never);
...
function NavItem({ item, isActive, onClick }: { item: typeof navGroups[0]['items'][0]; isActive: boolean; onClick?: () => void }) {
  return (
    <Link
      to={item.href}
      onClick={onClick}
  ...
  return (
    <div className="flex flex-col h-full bg-sidebar">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {buildNavGroups(user).map((group, i) => (
...
export function MainLayout() {
  ...
  const location = useLocation();
  const { user } = useAuth();
  const { tenants, selectedTenantId, selectTenant } = useTenants();
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center gap-4 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {user?.role === 'superadmin' && tenants.length > 0 && (
              <Select value={selectedTenantId ?? tenants[0].id} onValueChange={selectTenant}>
                <SelectTrigger className="hidden md:flex h-8 w-[190px] bg-muted text-xs">
