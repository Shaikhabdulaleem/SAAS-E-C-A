...
import { apiRequest, getSelectedTenantId, setSelectedTenantId as persistSelectedTenantId } from '../lib/api';
import { useAuth } from './AuthContext';

export type ServiceKey = 'crm' | 'email_marketing' | 'cold_email' | 'ai_assistant' | 'analytics' | 'api_access';
...
export const PLANS: Plan[] = [
  ...
  },
  {
    ...
    services: ['crm', 'email_marketing', 'cold_email', 'ai_assistant', 'analytics'],
    color: 'text-violet-700', bgColor: 'bg-violet-50',
  },
  {
    key: 'enterprise', label: 'Enterprise', price: 799, billingCycle: 'monthly',
    services: ['crm', 'email_marketing', 'cold_email', 'ai_assistant', 'analytics', 'api_access'],
...
export function TenantProvider({ children }: { children: ReactNode }) {
  ...
  const refreshTenants = async () => {
    ...
    if (user.role === 'client') {
      setSelectedTenantIdState(user.tenantId ?? null);
      persistSelectedTenantId(user.tenantId ?? null);
      setApiError(null);
      return;
    }
    ...
    try {
      ...
      const nextSelected = normalized.some(t => t.id === stored) ? stored : normalized[0]?.id ?? null;
      setSelectedTenantIdState(nextSelected);
      persistSelectedTenantId(nextSelected);
      setApiError(null);
    } catch (error) {
  ...
  const deleteTenant = (id: string) => {
    ...
    if (selectedTenantId === id) {
      const nextTenant = tenants.find(t => t.id !== id)?.id ?? null;
      setSelectedTenantIdState(nextTenant);
      persistSelectedTenantId(nextTenant);
    }
  ...
  return (
    <TenantContext.Provider value={{ tenants, selectedTenantId, selectedTenant, apiError, selectTenant, refreshTenants, addTenant, updateTenant, deleteTenant, addIntegration, updateIntegration, removeIntegration }}>
      {children}
