...
function normalizeWarmupPersona(persona: WarmupApiPersona, status?: string): WarmupPersona {
  const fullName = persona.fullName ?? [persona.firstName, persona.lastName].filter(Boolean).join(' ') ?? 'Unnamed persona';
  return {
    id: persona.id ?? '',
    fullName: fullName || 'Unnamed persona',
    email: persona.email ?? '',
    domain: persona.domain ?? '',
    warmupStatus: (persona.warmupStatus ?? status ?? 'not_started') as WarmupPersona['warmupStatus'],
    warmupDay: persona.warmupDay ?? 0,
    warmupTotal: persona.warmupTotal ?? 35,
    dailyLimit: persona.dailyLimit ?? persona.dailySendLimit ?? 0,
    sentToday: persona.sentToday ?? 0,
    healthScore: persona.healthScore ?? 0,
    bounceRate: persona.bounceRate ?? 0,
    spamRate: persona.spamRate ?? 0,
  };
}

function normalizeWarmupResponse(data: WarmupApiResponse): WarmupPersona[] {
  if (Array.isArray(data)) return data.map(persona => normalizeWarmupPersona(persona));
  if (!data?.groups) return [];

  return Object.entries(data.groups).flatMap(([status, personas]) =>
    Array.isArray(personas) ? personas.map(persona => normalizeWarmupPersona(persona, status)) : [],
  );
}
