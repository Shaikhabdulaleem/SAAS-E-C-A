...
import {
  useTenants, PLANS, ALL_SERVICES, INTEGRATION_PLATFORMS,
  type ServiceKey, type PlanKey, type TenantStatus, type IntegrationPlatformKey,
...
export function TenantDetail() {
  ...
  const { tenants, updateTenant, deleteTenant, addIntegration, updateIntegration, removeIntegration } = useTenants();
  ...
  const handlePlanChange = (planKey: PlanKey) => {
    ...
    updateTenant(tenant.id, {
      ...
      mrr: tenant.status === 'active' ? newPlan.price : tenant.mrr,
    });
  };

  const handleStatusChange = (status: TenantStatus) => {
    updateTenant(tenant.id, { status, mrr: status === 'active' ? plan.price : 0 });
  };
