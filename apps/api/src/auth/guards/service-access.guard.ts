...
export class ServiceAccessGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const selectedTenantId = typeof request.headers['x-tenant-id'] === 'string' ? request.headers['x-tenant-id'] : undefined;
    const isImpersonating = request.headers['x-admin-impersonation'] === 'true';
    const tenantId = user.role === UserRole.superadmin ? selectedTenantId : user.tenantId;

    if (user.role === UserRole.superadmin && (!isImpersonating || !tenantId)) {
      throw new ForbiddenException('Superadmin must open a client portal session before using tenant services');
    }
